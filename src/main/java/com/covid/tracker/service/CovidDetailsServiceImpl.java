...
public class CovidDetailsServiceImpl implements CovidDetailsService {
	...
	private void updateHistory(String apiName) {
		try {
			List<ApiHistory> history = apiHistoryRepository.findAllByApiName(apiName);
			if (CollectionUtils.isEmpty(history)) {
				apiHistoryRepository.save(new ApiHistory(apiName, new Date(), null));
			} else {
	...
	private void updateHistory(String apiName, String type) {
		try {
			List<ApiHistory> history = apiHistoryRepository.findAllByApiNameAndType(apiName, type);
			if (CollectionUtils.isEmpty(history)) {
				apiHistoryRepository.save(new ApiHistory(apiName, new Date(), type));
			} else {
				ApiHistory hist = history.get(0);
				hist.setDate(new Date());
				apiHistoryRepository.save(hist);
	...
	public List<CovidData> getCovidDataByName(String name,boolean fetchDb) {
		...
		try {
			if(fetchDb) {
				return Collections.singletonList(covidDataRepository.findByCountry(name));
			}
			if (isOutdated) {
	...
	public List<CovidData> getCovidDataByCode(String code,boolean fetchDb) {
		...
		try {
			if(fetchDb) {
				return Collections.singletonList(covidDataRepository.findByCode(code));
			}
			if (isOutdated) {
	...
	private boolean checkOutDated(String apiName) {
		List<ApiHistory> result = apiHistoryRepository.findAllByApiName(apiName);
	...
	private boolean checkOutDated(String apiName, String type) {
		List<ApiHistory> result = apiHistoryRepository.findAllByApiNameAndType(apiName, type);
		return CollectionUtils.isEmpty(result) ? true : getDiff(result.get(0).getDate()) > 7 ? true : false;
	}
